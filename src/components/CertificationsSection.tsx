const CertificationsSection = () => {
  const certifications = [
    ...
    },
    {
      title: "GMP Certified",
      description:
        "Sản xuất theo tiêu chuẩn quốc tế, quy trình kiểm soát nghiêm ngặt",
      icon: (
        <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      title: "ISO 22000",
      description:
        "Hệ thống quản lý an toàn thực phẩm, đảm bảo chất lượng ổn định",
      icon: (
        <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      title: "Lab-Tested",
      description:
        "Kiểm định bởi phòng thí nghiệm độc lập, không chất bảo quản",
      icon: (
        <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16 animate-fade-up">
          <h2 className="font-cinzel text-4xl lg:text-5xl font-bold text-primary mb-6">
            CHẤT LƯỢNG DƯỢC CÔNG NHẬN TOÀN CẦU
          </h2>
          <p className="font-montserrat text-lg text-muted-foreground max-w-2xl mx-auto">
            Các chứng nhận uy tín khẳng định chất lượng và độ an toàn của sản
            phẩm
          </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8 max-w-6xl mx-auto">
          {certifications.map((cert, index) => (
            <div
              key={index}
              className={`group text-center animate-fade-up animate-delay-${
                (index + 1) * 100
              }`}
            >
        ...
        <div className="text-center mt-16 animate-fade-up animate-delay-600">
          <div className="bg-gradient-to-r from-transparent via-secondary/10 to-transparent p-8 rounded-2xl border border-secondary/20">
            ...
            <p className="font-montserrat text-muted-foreground max-w-3xl mx-auto leading-relaxed">
              Mỗi sản phẩm Sâm Chạy Bộ đều được kiểm định gắt gao, đạt chuẩn
              quốc tế và chứng nhận an toàn tuyệt đối.
            </p>
          </div>
        </div>
      </div>
